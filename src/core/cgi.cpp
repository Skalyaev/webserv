...
std::string completeCGIheader(const std::string &header, short &code, const bool &cLenFind, const size_t &bodySize) {
        if (!bodySize)
                code = OK_NOCONTENT;
        std::string ret;
        ret += header;
...
bool parseCGIheader(const int &id, std::string &header, const std::string &body, std::string &answ, t_serv &serv, const int &socket) {
...
bool completeAnsw(const int &id, const std::string &temp, t_serv &serv, std::string &answ, const int &socket) {
        if (temp.empty()) {
                answ = createAnsw(std::string(), OK_NOCONTENT, data()->codes[OK_NOCONTENT], std::string(), std::string());
                return true;
        }
...
bool parentProcess(const int &id, const int &toWrite, const int &toRead, t_serv &serv, std::string &answ, const int &socket, const pid_t &pid) {
        if (!data()->bodies[id].second.empty()) {
                if (write(toWrite, &data()->bodies[id].second[0], data()->bodies[id].second.size()) != static_cast< ssize_t >(data()->bodies[id].second.size())) {
                        close(toWrite);
                        close(toRead);
...
// execve() le CGI
bool execCGI(const int &id, std::map< const std::string, std::string > &cgiEnv, const std::vector< char * > &argz, t_serv &serv, std::string &answ, const int &socket) {
        int pipeIN[2];
        int pipeOUT[2];
        if (pipe(pipeIN) < 0)
                return sendError(id, ERR_INTERN, data()->codes[ERR_INTERN], socket, serv);
...
bool getClientInfos(const int &id, const int &socket, std::string &ip, std::string &port, t_serv &serv) {
        t_sockaddr_in addr;
...
bool getScriptInfos(const int &id, const std::string &URI, std::string &scriptName, std::string &pathInfo, t_serv &serv, const int &socket) {
...
bool setEnv(const int &id, std::map< const std::string, std::string > &cgiEnv, t_serv &serv, const int &socket, const std::pair< uint32_t, uint16_t > &ip_port, const std::string &serverName, std::string &root) {
        std::map< const std::string, std::string >::iterator it;

        it = data()->headers[id].second.find("User-Agent");
        cgiEnv["HTTP_USER_AGENT"] = (it != data()->headers[id].second.end() ? it->second : std::string());
        ...
        cgiEnv["HTTP_CONNECTION"] = (it != data()->headers[id].second.end() ? it->second : std::string());
        ...
        cgiEnv["CONTENT_LENGTH"] = (it != data()->headers[id].second.end() ? it->second : std::string());
        ...
        cgiEnv["SCRIPT_FILENAME"] = std::string(getcwd(buffer, BUFFER_SIZE)) + root.substr(1, root.size()) + scriptName;
