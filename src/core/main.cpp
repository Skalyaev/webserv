...
bool bind2socket(const size_t &x) {
        int on = 1;
        if (setsockopt(data()->sock_addr[x].first, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast< char * >(&on), sizeof(on)) < 0) {
        ...
        if (bind(data()->sock_addr[x].first, reinterpret_cast< t_sockaddr * >(&(data()->sock_addr[x].second)), sizeof(data()->sock_addr[x].second)) < 0) {
...
bool setup(const size_t &x, const bool &reUP) {
        if (!reUP)
                data()->sock_addr.push_back(std::pair< const int, t_sockaddr_in >(socket(AF_INET, SOCK_STREAM, 0), t_sockaddr_in()));
        else
                data()->sock_addr[x] = std::pair< const int, t_sockaddr_in >(socket(AF_INET, SOCK_STREAM, 0), t_sockaddr_in());
        ...
        if (reUP) {
                std::cout << "Listenin socket #" << x << " repaired. Listenin: " << ntohl(data()->sock_addr[x].second.sin_addr.s_addr) << ":" << ntohs(data()->sock_addr[x].second.sin_port) << std::endl;
        ...
        std::cout << "Listenin : " << ntohl(data()->sock_addr[x].second.sin_addr.s_addr) << ":" << ntohs(data()->sock_addr[x].second.sin_port) << std::endl;
        return true;
}
