...
void printLocation(t_location &ref) {
        ...
        for (size_t a = 0; a < ref.limitExcept["GET"].size(); a++)
                std::cout << "\t" << ref.limitExcept["GET"][a].first << " -> " << ref.limitExcept["GET"][a].second << std::endl;

        std::cout << "- POST: " << std::endl;
        for (size_t b = 0; b < ref.limitExcept["POST"].size(); b++)
                std::cout << "\t" << ref.limitExcept["POST"][b].first << " -> " << ref.limitExcept["POST"][b].second << std::endl;

        std::cout << "- DELETE: " << std::endl;
        for (size_t c = 0; c < ref.limitExcept["DELETE"].size(); c++)
                std::cout << "\t" << ref.limitExcept["DELETE"][c].first << " -> " << ref.limitExcept["DELETE"][c].second << std::endl;
