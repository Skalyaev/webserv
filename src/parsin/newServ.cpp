...
bool setIpPort(const uint32_t &ip, const uint16_t &port, std::vector< std::pair< uint32_t, uint16_t > > &ref) {
        ...
        for (size_t x = 0; x < data()->ip_port.size(); x++)
                if ((data()->ip_port[x].first == ip && data()->ip_port[x].second == port) || (data()->ip_port[x].first == 0 && data()->ip_port[x].second == port))
                        return true;
        if (ip == 0)
                for (size_t x = 0; x < data()->ip_port.size(); x++)
                        if (data()->ip_port[x].second == port)
                                data()->ip_port.erase(data()->ip_port.begin() + x--);
        data()->ip_port.push_back(std::pair< uint32_t, uint16_t >(ip, port));
        return true;
}
...
bool listenIsHere(std::istringstream &stream, std::string &word, t_serv &serv, const std::string &keyword, const std::ostringstream &lineIndex) {
        if (!(stream >> word))
                return configError("[ ERROR ] listenIsHere() - " + keyword + " [" + lineIndex.str() + ']' + S_ERR_NO_VAL, false, 0);
        if (!getIpPort(word, serv.ip_port))
                return configError("[ ERROR ] listenIsHere() - " + keyword + " [" + lineIndex.str() + ']' + S_ERR_BAD_VAL, false, 0);
        if (stream >> word)
                return configError("[ ERROR ] listenIsHere() - " + keyword + " [" + lineIndex.str() + ']' + S_ERR_TM_VAL, false, 0);
        return true;
}

// La directive serverName a été trouvé dans un bloc serveur
bool serverNameIsHere(std::istringstream &stream, std::string &word, t_serv &serv, const std::string &keyword, const std::ostringstream &lineIndex) {
        if (!(stream >> word))
                return configError("[ ERROR ] serverNameIsHere() - " + keyword + " [" + lineIndex.str() + ']' + S_ERR_NO_VAL, false, 0);
        while (!stream.fail()) {
...
bool errorPageIsHere(std::istringstream &stream, std::string &word, t_serv &serv, const std::string &keyword, const std::ostringstream &lineIndex) {
        if (!(stream >> word))
                return configError("[ ERROR ] errorPageIsHere() - " + keyword + " [" + lineIndex.str() + ']' + S_ERR_NO_VAL, false, 0);
        while (!stream.fail()) {
                ...
                if (codeStr.find_first_not_of("0123456789") != std::string::npos)
                        return configError("[ ERROR ] errorPageIsHere() - " + keyword + " [" + lineIndex.str() + ']' + S_ERR_BAD_VAL, false, 0);
...
bool parseServSwitch(std::istringstream &stream, std::string &word, t_serv &serv, std::ifstream &config, size_t &configLine, const std::ostringstream &lineIndex, int &maxBodySize, bool &stop, bool &stopBis) {
        ...
        case (6):
                ...
                break;
        default:
                return configError("[ ERROR ] parseServSwitch() - Unknown keyword [" + lineIndex.str() + "]: " + word, false, 0);
        }
        return true;
}
...
bool parseServ(std::ifstream &config, size_t &configLine) {
        ...
        while (!stop && std::getline(config, line)) {
                ...
                while (!stopBis && !stream.fail() && (stream >> word))
                        if (!parseServSwitch(stream, word, serv, config, configLine, lineIndex, maxBodySize, stop, stopBis))
                                return false;
        }
        if (!stop)
