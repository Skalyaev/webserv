...
void setMIMEtypes(void) {
        // Texte à afficher
        data()->MIMEtypes.insert(std::pair< const std::string, const std::string >("plain", "text/plain"));
        data()->MIMEtypes.insert(std::pair< const std::string, const std::string >("html", "text/html"));
        data()->MIMEtypes.insert(std::pair< const std::string, const std::string >("css", "text/css"));

        // Application à exécuter
        data()->MIMEtypes.insert(std::pair< const std::string, const std::string >("php", "application/x-httpd-php"));
        data()->MIMEtypes.insert(std::pair< const std::string, const std::string >("py", "application/x-python"));
        data()->MIMEtypes.insert(std::pair< const std::string, const std::string >("pdf", "application/pdf"));
        data()->MIMEtypes.insert(std::pair< const std::string, const std::string >("xml", "application/xml"));
        data()->MIMEtypes.insert(std::pair< const std::string, const std::string >("json", "application/json"));
        data()->MIMEtypes.insert(std::pair< const std::string, const std::string >("js", "application/javascript"));

        // Média
        data()->MIMEtypes.insert(std::pair< const std::string, const std::string >("ico", "image/vnd.microsoft.icon"));
        data()->MIMEtypes.insert(std::pair< const std::string, const std::string >("jpeg", "image/jpeg"));
        ...
        data()->MIMEtypes.insert(std::pair< const std::string, const std::string >("mpeg", "audio/mpeg"));
...
void setDefCodes(void) {
        ...
        // 3xx
        data()->codes.insert(std::pair< const short, const std::string >(RE_MOVED, "Moved Permanently"));
        data()->codes.insert(std::pair< const short, const std::string >(RE_FOUND, "Found"));
        data()->codes.insert(std::pair< const short, const std::string >(RE_TEMP, "Temporary Redirect"));
        data()->codes.insert(std::pair< const short, const std::string >(RE_PERM, "Permanent Redirect"));
        ...
        data()->codes.insert(std::pair< const short, const std::string >(ERR_UNALW_METHOD, "Method Not Allowed"));
        data()->codes.insert(std::pair< const short, const std::string >(ERR_NOT_ACCEPT, "Not Acceptable"));
        data()->codes.insert(std::pair< const short, const std::string >(ERR_REQ_TIMEOUT, "Request Timeout"));
        data()->codes.insert(std::pair< const short, const std::string >(ERR_LEN_REQ, "Length Required"));
        data()->codes.insert(std::pair< const short, const std::string >(ERR_BODY_2LARG, "Payload Too Large"));
        data()->codes.insert(std::pair< const short, const std::string >(ERR_URI2LONG, "URI Too Long"));
        data()->codes.insert(std::pair< const short, const std::string >(ERR_UNKN_TYPE, "Unsupported Media Type"));
        data()->codes.insert(std::pair< const short, const std::string >(ERR_HEADER2LARG, "Request Header Fields Too Large"));

        // 5xx
        data()->codes.insert(std::pair< const short, const std::string >(ERR_INTERN, "Internal Server Error"));
        data()->codes.insert(std::pair< const short, const std::string >(ERR_UNKN_METHOD, "Not Implemented"));
        data()->codes.insert(std::pair< const short, const std::string >(ERR_BAD_GATE, "Bad Gateway"));
        data()->codes.insert(std::pair< const short, const std::string >(ERR_HTTP_VERSION, "HTTP Version Not Supported"));
}
...
bool maxBodySizeIsHere(int &maxBodySize, std::istringstream &stream, std::string &word, const std::string &keyword, const std::ostringstream &lineIndex) {
        if (maxBodySize != -1)
                return configError("[ ERROR ] maxBodySizeIsHere() - " + keyword + " [" + lineIndex.str() + ']' + S_ERR_TM_VAL, false, 0);
        if (!(stream >> word))
                return configError("[ ERROR ] maxBodySizeIsHere() - " + keyword + " [" + lineIndex.str() + ']' + S_ERR_NO_VAL, false, 0);
        if ((maxBodySize = getmaxBodySize(word)) < 0)
                return configError("[ ERROR ] maxBodySizeIsHere() - " + keyword + " [" + lineIndex.str() + ']' + S_ERR_BAD_VAL, false, 0);
        if (stream >> word)
                return configError("[ ERROR ] maxBodySizeIsHere() - " + keyword + " [" + lineIndex.str() + ']' + S_ERR_TM_VAL, false, 0);
        return true;
}
...
bool serverBlockIsHere(std::istringstream &stream, std::string &word, std::ifstream &config, size_t &configLine, const std::string &keyword, const std::ostringstream &lineIndex) {
        if (!(stream >> word))
                return configError("[ ERROR ] serverBlockIsHere() - " + keyword + " [" + lineIndex.str() + ']' + S_ERR_NO_VAL, false, 0);
        if (word != "{")
                return configError("[ ERROR ] serverBlockIsHere() - " + keyword + " [" + lineIndex.str() + ']' + S_ERR_NO_BRK, false, 0);
        if (stream >> word)
                return configError("[ ERROR ] serverBlockIsHere() - " + keyword + " [" + lineIndex.str() + ']' + S_ERR_TM_VAL, false, 0);
...
bool initSwitch(std::istringstream &stream, std::string &word, std::ifstream &config, size_t &configLine, const std::ostringstream &lineIndex, int &maxBodySize, bool &stop) {
        ...
        case (2):
                ...
                break;
        default:
                return configError("[ ERROR ] initSwitch() - Unknown keyword [" + lineIndex.str() + "]: " + word, false, 0);
        }
        return true;
}
