...
void printRequest(std::map< const std::string, std::string > &header, std::vector< char > &body) {
        for (std::map< const std::string, std::string >::iterator it = header.begin(); it != header.end(); it++)
...
bool isItOver(const size_t &x, const int &id, const size_t &servIndex, const int &socket, const int &contentLen, const short &chunked, const size_t &bodySize, std::map< const std::string, std::string > &header) {
        std::cout << "chunked " << chunked << std::endl;
        if (chunked == 1 || (!chunked && bodySize < static_cast< size_t >(contentLen))) {
                std::cout << "2.25" << std::endl;
                if (header["Method"] == "GET")
                        return sendError(id, ERR_BAD_REQ, data()->codes[ERR_BAD_REQ], socket, data()->servList[servIndex]);
                try2keep(socket);
                if (!data()->pollz.second[x].second) {
                        data()->pollz.second[x].second = new t_timeval;
                        if (!data()->pollz.second[x].second || gettimeofday(data()->pollz.second[x].second, NULL))
                                return sendError(id, ERR_INTERN, data()->codes[ERR_INTERN], socket, data()->servList[servIndex]);
                }
                const std::map< const std::string, std::string >::iterator it = header.find("Expect");
                if (it != header.end() && it->second == "100-continue") {
                        const std::string answ = createAnsw(std::string(), INFO_CONTINUE, data()->codes[INFO_CONTINUE], std::string(), std::string());
...
bool unchunk(std::vector< char > &body, const int &id, const size_t &servIndex, const int &socket, short &unchunk) {
...
short appendBodyBuffer(std::vector< char > &body, size_t &bodySize, ssize_t &readed, const int &socket, bool &stop) {
...
bool read2body(const size_t &x, const int &id, const int &socket, ssize_t &readed, const size_t &servIndex, std::map< const std::string, std::string > &header, std::vector< char > &body) {
        int contentLen = 0;
        short chunked = 0;
        ...
        if (contentLen > data()->servList[servIndex].maxBodySize * ONE_MO)
                return sendError(id, ERR_BODY_2LARG, data()->codes[ERR_BODY_2LARG], socket, data()->servList[servIndex]);
        size_t bodySize = body.size();
        bool stop = false;
        while (!stop && readed == BUFFER_SIZE && bodySize < static_cast< size_t >(data()->servList[servIndex].maxBodySize * ONE_MO))
                if (!appendBodyBuffer(body, bodySize, readed, socket, stop))
                        return false;

        if (!contentLen && !chunked && bodySize)
                return sendError(id, ERR_LEN_REQ, data()->codes[ERR_LEN_REQ], socket, data()->servList[servIndex]);
...
bool readHeaderLine(std::map< const std::string, std::string > &header, const std::string &line, const int &id, bool &bodyInc, size_t &servIndex, const int &socket) {
        if (line == "\r" && (bodyInc = true))
                return true;
        std::pair< std::string, std::string > values = ft::psplit(line, ':');
        if (values.first.empty())
                return sendError(id, ERR_BAD_REQ, data()->codes[ERR_BAD_REQ], socket, data()->servList[servIndex]);
        const std::string key = values.first.substr(values.first.find_first_not_of(" \t"), values.first.find_last_not_of(" \t") - values.first.find_first_not_of(" \t") + 1);
        const std::string value = values.second.substr(values.second.find_first_not_of(" \t"), values.second.find_last_not_of(" \t") - values.second.find_first_not_of(" \t") + 1);
...
void look4subLoca(const std::string &URI, t_location &location, std::map< const std::string, std::string > &header) {
...
bool locaIndexFromURI(const std::string &URI, t_serv &serv, t_location &location, std::map< const std::string, std::string > &header) {
...
bool localise2servBlock(std::map< const std::string, std::string > &header, std::istringstream &stream, size_t &servIndex, const std::pair< uint32_t, uint16_t > &ip_port, std::string &serverName) {
        bool ret = true;
        std::string key;
        std::string value;
        std::string line;
        if (std::getline(stream, line)) {
                std::pair< std::string, std::string > values = ft::psplit(line, ':');
                key = values.first.substr(values.first.find_first_not_of(" \t"), values.first.find_last_not_of(" \t") - values.first.find_first_not_of(" \t") + 1);
                value = values.second.substr(values.second.find_first_not_of(" \t"), values.second.find_last_not_of(" \t") - values.second.find_first_not_of(" \t") + 1);
                if (key != "Host")
                        ret = false;
                else
                        header["Host"] = value;
        ...
        for (size_t x = 0; x < data()->servList.size() && !stop; x++) {
                for (size_t y = 0; y < data()->servList[x].ip_port.size(); y++) {
                        if ((data()->servList[x].ip_port[y].first == ip_port.first && data()->servList[x].ip_port[y].second == ip_port.second) || (ip_port.first == 0 && data()->servList[x].ip_port[y].second == ip_port.second)) {
                                if (!defaultServ)
                                        defaultServ = x + 1;
...
unsigned short check1stLine(std::istringstream &stream, std::map< const std::string, std::string > &header) {
        std::string line;
        if (std::getline(stream, line)) {
...
bool check2header(const std::string &fullBuffer, const int &id, std::map< const std::string, std::string > &header, const std::pair< uint32_t, uint16_t > &ip_port, size_t &servIndex, t_location &location, std::string &serverName, const int &socket, size_t &totalReaded, size_t &bodyInc) {
...
// Lit le header
bool read2header(const size_t &x, const int &id, const int &socket, ssize_t &readed, std::map< const std::string, std::string > &header, std::vector< char > &body, size_t &servIndex, t_location &location, std::string &serverName) {
        char buffer[BUFFER_SIZE + 1];
        ...
        while (readed == BUFFER_SIZE && bodyInc == std::string::npos && totalReaded < HEADER_MAX_SIZE) {
                ...
                if (readed < 0) {
                        keepNoMore(socket);
                        std::cerr << "[ ERROR ] read2socket - read() failed for " << socket << "..." << std::endl;
        ...
        const std::pair< uint32_t, uint16_t > &ip_port = data()->ip_port[data()->pollz.second[x].first];
        return check2header(fullBuffer, id, header, ip_port, servIndex, location, serverName, socket, totalReaded, bodyInc);
}
