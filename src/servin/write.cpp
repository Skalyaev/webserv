...
bool fillBody(const int &id, std::string &type, const std::string &path, t_serv &serv, std::string &answ, const int &socket) {
        const std::map< const std::string, const std::string >::iterator mimeIT = data()->MIMEtypes.find(type);
        ...
        std::map< const std::string, std::string >::iterator headerIT = data()->headers[id].second.find("Accept");
        if (headerIT != data()->headers[id].second.end() && headerIT->second.find(type) == std::string::npos && headerIT->second.find(splited[0] + "/*") == std::string::npos && headerIT->second.find("*/" + splited[1]) == std::string::npos && headerIT->second.find("*/*") == std::string::npos)
...
bool sendDefaultError(const int &id, const unsigned short &codeRef, const std::string &message, const int &socket) {
        ...
        const std::string body = "<!DOCTYPE html><meta charset=\"utf-8\"><html><head><title>" + code2str.str() +
        ...
        if (write(socket, answ.c_str(), answ.size()) != static_cast< ssize_t >(answ.size())) {
                keepNoMore(socket);
                std::cerr << "[ ERROR ] sendDefaultError - write() failed to socket " << socket << std::endl;
        }
        return false;
}

// Envoie une réponse d'erreur HTTP
bool sendError(const int &id, const unsigned short codeRef, const std::string &message, const int &socket, t_serv &serv) {
        if (serv.errorPages.find(codeRef) == serv.errorPages.end())
                return sendDefaultError(id, codeRef, message, socket);

        const std::map< const std::string, std::string >::iterator URI = data()->headers[id].second.find("URI");
        std::string answ;
        redirect(answ, std::pair< short, std::string >(RE_MOVED, data()->headers[id].second["Host"] + "/" + serv.errorPages[codeRef]), URI);
        if (write(socket, answ.c_str(), answ.size()) != static_cast< ssize_t >(answ.size())) {
                keepNoMore(socket);
