...
bool checkMethodAuth(t_location &loca, const std::string &method, uint32_t &hostIP) {
...
bool answ2request(const size_t &x, const int &id, std::string &answ, t_serv &serv, t_location &location, const std::string &serverName) {
        ...
        uint32_t ip = (ret < 0 ? data()->ip_port[data()->pollz.second[x].first].first : ret);
        uint32_t port = data()->ip_port[data()->pollz.second[x].first].second;
        std::pair< uint32_t, uint16_t > ip_port(ip, port);
...
void resolveInc(const size_t &x) {
        ...
        if (requestID < 0) {
                data()->headers.push_back(std::pair< int, std::map< const std::string, std::string > >(data()->pollz.first[x].fd, std::map< const std::string, std::string >()));
                data()->bodies.push_back(std::pair< int, std::vector< char > >(data()->pollz.first[x].fd, std::vector< char >()));
        ...
        if (read2socket(x, requestID, servIndex, location, serverName)) {
                if (answ2request(x, requestID, answ, data()->servList[servIndex], location, serverName)) {
                        if (write(data()->pollz.first[x].fd, answ.c_str(), answ.size()) != static_cast< ssize_t >(answ.size())) {
                                keepNoMore(data()->pollz.first[x].fd);
                                std::cerr << "[ ERROR ] resolveInc() - write() failed to " << data()->pollz.first[x].fd << std::endl;
                        }
                        data()->headers.erase(data()->headers.begin() + requestID);
                        data()->bodies.erase(data()->bodies.begin() + requestID);
                }
                ...
                if (requestID >= 0) {
                        std::map< const std::string, std::string >::iterator it(data()->headers[requestID].second.find("Method"));
                        if (it != data()->headers[requestID].second.end() && it->second == "POST") {
                                if (data()->headers[requestID].second.find("Chunked") != data()->headers[requestID].second.end() && !data()->bodies[requestID].second.empty()) {
                                        std::pair< uint32_t, uint16_t > &ip_port = data()->ip_port[data()->pollz.second[x].first];
                                        if (!checkMethodAuth(location, "POST", ip_port.first))
                                                sendError(requestID, ERR_UNALW_METHOD, data()->codes[ERR_UNALW_METHOD], data()->pollz.first[x].fd, data()->servList[servIndex]);
                                        else
                                                ftPost(requestID, data()->servList[servIndex], location, answ, data()->pollz.first[x].fd, ip_port, serverName, true);
                                }
                        }
                }
        }
...
void acceptInc(const size_t &x) {
        ...
        while (true) {
                newSocket.fd = accept(data()->pollz.first[x].fd, reinterpret_cast< t_sockaddr * >(&data()->sock_addr[x].second), &addrlen);
                if (newSocket.fd < 0) {
                        if (errno != EWOULDBLOCK) {
                                std::cerr << "[ ERROR ] acceptInc() - accept() failed on ";
                                std::cerr << ntohl(data()->sock_addr[x].second.sin_addr.s_addr) << ":" << ntohs(data()->sock_addr[x].second.sin_port) << std::endl;
...
void socketError(const size_t &x) {
        if (x < data()->sock_addr.size()) {
                std::cerr << "[ ERROR ] socketError() - Listenin socket " << data()->pollz.first[x].fd << " broken, rebuilding..." << std::endl;
                close(data()->pollz.first[x].fd);
                if (!setup(x, true)) {
                        std::cerr << "[ ERROR ] socketError() - Can't reset " << data()->pollz.first[x].fd << ", ";
                        std::cerr << ntohl(data()->sock_addr[x].second.sin_addr.s_addr) << ":" << ntohs(data()->sock_addr[x].second.sin_port);
                ...
                std::cerr << "[ ERROR ] socketError() - Communication socket " << data()->pollz.first[x].fd << " broken, closin it" << std::endl;
                closeSocket(x);
        }
}
...
void checkTimeout(void) {
        ...
        size_t x = -1;
        while (++x < size) {
                if (data()->pollz.second[x].second && !data()->pollz.first[x].revents && (failed || (time.tv_sec - data()->pollz.second[x].second->tv_sec) >= REQ_TIMEOUT)) {
                        delete data()->pollz.second[x].second;
                        const int id = getRequestID(data()->pollz.first[x].fd);
                        t_serv &serv = data()->servList[std::atoi(data()->headers[id].second["servIndex"].c_str())];
                        const short code = (failed ? ERR_INTERN : ERR_REQ_TIMEOUT);
                        sendError(getRequestID(data()->pollz.first[x].fd), code, data()->codes[code], data()->pollz.first[x].fd, serv);
...
bool serv(void) {
        ...
        while (!data()->pollz.first.empty()) {
                ret = poll(data()->pollz.first.data(), static_cast< nfds_t >(data()->pollz.first.size()), pollTimeout);
                if (ret < 0) {
                        if (!data()->exiting)
