...
bool autoIndexON(const int &id, std::string &path, t_serv &serv, std::string &answ, const int &socket, const std::string &root) {
        ...
        if ((dir = opendir(path.c_str())) != NULL) {
                ...
                while ((ent = readdir(dir)) != NULL) {
                        ...
                        if (S_ISDIR(fileStat.st_mode))
                                fileName += "/";
                        html << "<tr><td><a href=\"" << relativePath << fileName << "\">" << fileName << "</a></td></tr>";
                }
                html << "</table>"
                     << "</div></body></html>";
        } else {
...
bool redirect(std::string &answ, std::pair< short, std::string > redir, const std::map< const std::string, std::string >::iterator &URI) {
        ...
        answ = createAnsw(std::string(), redir.first, data()->codes[redir.first], std::string(), "http://" + redir.second);
        return true;
}

// Cree un index par defaut
bool sendDefaultIndex(std::string &answ) {
        const std::string body = "<!DOCTYPE html><html><head><meta charset=\"utf-8\"><title>Default "
...
bool dirRequested(const int &id, t_serv &serv, t_location &location, std::string &answ, const int &socket, std::string &path, short &ret, const std::map< const std::string, std::string >::iterator &URI, const std::string &root) {
        ...
        if (location.name == "/")
                ...
                return redirect(answ, std::pair< short, std::string >(RE_MOVED, data()->headers[id].second["Host"] + URI->second + '/' + indexName), URI);
        return true;
}
