...
bool saveFile(const int &id, const int &socket, const std::string &filename) {
        int output = open(filename.c_str(), O_WRONLY | O_CREAT | O_APPEND, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
        if (output < 0)
                return sendDefaultError(id, ERR_INTERN, data()->codes[ERR_INTERN], socket);

        if (!data()->bodies[id].second.empty()) {
                const ssize_t count = write(output, &data()->bodies[id].second[0], data()->bodies[id].second.size());
                close(output);
...
// multipart/form-data
bool freeChunkedBody(const int &id, const int &socket, const std::string &path, const std::string &name) {
        const std::map< const std::string, std::string >::iterator it = data()->headers[id].second.find("Chunked");
        if (it->second == "1") {
                if (name.empty())
...
bool isFormData(const int &id, std::string &name, const std::map< const std::string, std::string >::iterator &it) {
        size_t pos = it->second.find("boundary=");
        if (pos == std::string::npos)
                return false;

        const std::string boundary = "--" + it->second.substr(pos + 9);
        const std::string temp = std::string(data()->bodies[id].second.begin(), data()->bodies[id].second.end());
...
bool isXFrom(const int &id, std::string &name) {
        const std::vector< std::string > splited = ft::split(std::string(data()->bodies[id].second.begin(), data()->bodies[id].second.end()), '&');
        if (splited.size() != 2)
                return false;
...
bool managedMIMES(const int &id, const std::string &path, std::string &answ, const int &socket, const bool &chunked) {
        std::string type;
        const std::map< const std::string, std::string >::iterator typeIT = data()->headers[id].second.find("Content-Type");
        if (typeIT == data()->headers[id].second.end())
                type = "application/octet-stream";
        else
                type = typeIT->second;

        const std::string keywords[] = {"application/octet-stream", "text/plain", "multipart/form-data", "application/x-www-form-urlencoded"};
        const std::vector< std::string > keys(keywords, keywords + sizeof(keywords) / sizeof(std::string));
        std::string name;
        switch (std::distance(keys.begin(), std::find(keys.begin(), keys.end(), ft::split(type, ';').front()))) {
        case (0):
        ...
        if (!saveFile(id, socket, path + '/' + name))
                return false;
        answ = createAnsw(std::string(), OK_CREATED, data()->codes[OK_CREATED], std::string(), std::string());
        return true;
}
